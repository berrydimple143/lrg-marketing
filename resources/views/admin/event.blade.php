	...
	<div class="container">
		<div class="mws-panel grid_8">
			<div class="mws-panel-header">
		    	<span><i class="icon-table"></i> Events Tasks
		            <div id="mws-searchbox" class="mws-inset pull-right" style="width: 300px;">
		                {!! Form::open(['route' => 'event.search', 'method' => 'POST']) !!}
		    ...
		    <div class="mws-panel-toolbar">
		        <div class="btn-toolbar">
		            <div class="btn-group">
		                <a href="{{ route('event.add') }}" class="btn"><i class="icol-add"></i> Add</a>                
		                <a href="#" class="btn"><i class="icol-printer"></i> Print</a>
		                <a href="{{ route('event') }}" class="btn"><i class="icol-arrow-refresh"></i> Refresh</a>                
		            </div>            
		        </div>
		    </div>    
		    <div class="mws-panel-body no-padding">
		        <table class="mws-table">
		            ...
		            <tbody>            
		            	@forelse($model as $evt)    
		                <tr>		                	
		                    <td>{{ $evt->customer_id }}</td>
		                    <td>{{ $evt->business_name }}</td>
		                    <td>
		                    	<center><a href="{{ $evt->link }}" target="_blank">		                    		
		                    		<button type="button" class="btn btn-primary btn-small"><i class="icon-search"></i> View Event</button>
		                    	</a></center>
		                    </td>		                    
		                    <?php
		                    	$date = new Carbon\Carbon($evt->date_created);
		                    	$dc = $date->toFormattedDateString();
		                    ?> 
		                    <td>{{ $dc }}</td>
		                    <td>{{ $evt->status }}</td>
		                    <td style="text-align: center;">
		                    	@if($evt->jstat == "Completed")
		                    ...
		                    <td style="text-align: center;">
		                        <span class="btn-group">		                            
		                            <a href="{{ route('event.edit', ['id' => $evt->id]) }}" class="btn btn-small"><i class="icon-pencil"></i></a>
